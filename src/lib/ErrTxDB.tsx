...
class MyIndexedDB {
  ...
  checkTable() {
    if (!this.db.objectStoreNames.contains(this.table)) {
      this.db.createObjectStore(this.table, { keyPath: 'tx' });
    }
  }
  ...
  async getItem(key: string): Promise<ErrTxDto | null> {
    await this.mounted;
    return new Promise((resolve, reject) => {
...
export const MyErrTxDB = new MyIndexedDB();
