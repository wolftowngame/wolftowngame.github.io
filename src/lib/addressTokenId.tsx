export interface addressTokenIdDto {
  tokenId: string;
  tx: string;
  from: string;
  type: 'Unknow' | 'Sheep' | 'Wolf';
}
class MyIndexedDB {
  db!: IDBDatabase;
  table = `addressTokenId`;
  name = `addressTokenId`;
  mounted: Promise<any>;
  constructor() {
  ...
  checkTable() {
    if (!this.db.objectStoreNames.contains(this.table)) {
      this.db.createObjectStore(this.table, { keyPath: 'tokenId' });
    }
  }
  ...
  async getItem(key: string): Promise<addressTokenIdDto> {
    await this.mounted;
    return new Promise((resolve, reject) => {
...
export const MyaddressTokenIdDB = new MyIndexedDB();
