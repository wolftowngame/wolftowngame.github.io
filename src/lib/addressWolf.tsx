export interface AddressWolfDto {
  from: string;
  tokenIds: string[];
  wolf: number;
  sheep: number;
...
class MyIndexedDB {
  ...
  checkTable() {
    if (!this.db.objectStoreNames.contains(this.table)) {
      this.db.createObjectStore(this.table, { keyPath: 'from' });
    }
  }
  ...
  async getItem(key: string): Promise<AddressWolfDto> {
    await this.mounted;
    return new Promise((resolve, reject) => {
...
export const MyAddressWolfDB = new MyIndexedDB();
